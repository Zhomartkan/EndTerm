...
import com.example.Hosp_System.Entity.Doctors;
...
public class Service {
    ...
    public Doctors doctorsLogin(String email, String password) {
        Doctors doctor = null;
        Connection con = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
            Class.forName(DatabaseConnection.DRIVERCLASS);
            con = DriverManager.getConnection(DatabaseConnection.DBURL, DatabaseConnection.USER, DatabaseConnection.PASSWORD);
            pstmt = con.prepareStatement("select * from docotors where email = ? and password = ? ");
            pstmt.setString(1, email);
            pstmt.setString(2, password);
            rs = pstmt.executeQuery();
            if (rs.next()) {
    ...
    public boolean addDoctors(Doctors doctors) {
        ...
        try {
            ...
            con = DriverManager.getConnection(DatabaseConnection.DBURL, DatabaseConnection.USER, DatabaseConnection.PASSWORD);
            pstmt = con.prepareStatement("insert into admins values (?,?,?,?)");
            pstmt.setInt(1, doctors.getId());
            pstmt.setString(2, doctors.getName());

            pstmt.setString(3, doctors.getEmail());
            pstmt.setString(4, doctors.getPassword());
    ...
    public Patient patientLogin(String email, String password) {
        ...
        try {
            Class.forName(DatabaseConnection.DRIVERCLASS);
            con = DriverManager.getConnection(DatabaseConnection.DBURL, DatabaseConnection.USER, DatabaseConnection.PASSWORD);
            pstmt = con.prepareStatement("select * from patient where email = ? and password = ? ");
            pstmt.setString(1, email);
            pstmt.setString(2, password);
            rs = pstmt.executeQuery();
            if (rs.next()) {
    ...
    public List<Patient> listAllPatient() {
        ...
        try {
            Class.forName(DatabaseConnection.DRIVERCLASS);
            con = DriverManager.getConnection(DatabaseConnection.DBURL, DatabaseConnection.USER, DatabaseConnection.PASSWORD);
            cal = con.prepareStatement("SELECT * FROM patient ;");
            state = cal.execute();
            if (state) {
                rs = cal.getResultSet();
                while (rs.next()) {
